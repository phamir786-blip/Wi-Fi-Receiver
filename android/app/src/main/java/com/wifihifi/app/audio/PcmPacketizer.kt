...
/**
 * Packs continuous PCM audio stream into framed UDP packets.
...
class PcmPacketizer(
    ...
    // 32-byte header + 1764-byte payload = 1796-byte UDP datagram.
    companion object {
        const val SAMPLES_PER_PACKET = 441
        const val PAYLOAD_BYTES_PER_PACKET = SAMPLES_PER_PACKET * WiFiHiFiProtocol.BYTES_PER_FRAME
        const val TOTAL_PACKET_CAPACITY = WiFiHiFiProtocol.HEADER_SIZE + PAYLOAD_BYTES_PER_PACKET
    }
    ...
    fun feedPcm(data: ByteArray, offset: Int, length: Int) {
        ...
        while (bytesProcessed < length) {
            ...
            if (accumulator.position() >= PAYLOAD_BYTES_PER_PACKET) {
                ...
                // Header is 32 bytes; payload starts immediately after it.
